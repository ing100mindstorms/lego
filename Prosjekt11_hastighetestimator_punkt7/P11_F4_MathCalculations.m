...
if(LysDerivert(k-1)< -1000 && LysDerivert(k-2) > -1000) %%Sjekker om det er ny stripe
    tidspunkt(end + 1) = Tid(k);
    hastighet(end + 1) = lengde / (tidspunkt(end)-tidspunkt(end-1)); %Beregner hastighet utifra avbruddsignal fra lyssensor
end

motorHastighetB(k) = motorSpeedCalculator(PosMotorB(k-1:k), Tid(k-1:k), radius); %Beregner hastighet utifra motorposisjon
motorHastighetC(k) = motorSpeedCalculator(PosMotorC(k-1:k), Tid(k-1:k), radius); %Beregner hastighet utifra motorposisjon
